...
export class HrmsComponent implements OnInit {
  ...
  Ryts:any= {};
  authBoolean:boolean ;
  showAssessmentSuite: boolean = false;
  ...
  ngOnInit(): void {
   // this.loggedUser = atob(localStorage.getItem('userData'))
    ...
    this.checkAssessmentAccess();
  }
  checkAssessmentAccess(): void {
    if (!this.Ryts) {
      this.showAssessmentSuite = false;
      return;
    }
  
    this.showAssessmentSuite = this.Ryts.some(x =>
      x.Initiate_Assessment_Process === 'true' ||
      x.Assessment_Approvals_Report === 'true' ||
      x.Assessment_Extended_Report === 'true' ||
      x.Assessment_Permant_Report === 'true'
    );
  }
  getApiData(){
