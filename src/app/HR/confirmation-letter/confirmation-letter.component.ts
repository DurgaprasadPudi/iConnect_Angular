...
this.authservice.getconfirmationregistation(requestData).subscribe(
  (response) => {
    ...
    // Reset the form and display data
    this.onReset();
    this.displayData();
  },
  (error) => {
    ...
    if (error.status === 400) {
      const errorMessage = error.error?.message || "An error occurred while processing your request. Please try again.";
      Swal.fire({
        title: "Error!",
        text: errorMessage,
        icon: "error",
      ...
      const errorMessage = error.error?.message || "An internal server error occurred. Please try again later.";
      Swal.fire({
        title: "Error!",
        text: errorMessage,
        icon: "error",
