...
myCarouselOptions={
  ...
  fetchForm16ByYear() {
    ...
    const pan = this.userData.user.pan;
    this.isLoading=true;
    this.authService.getForm16Pdf(this.selectedYear, pan).subscribe({
      next: (responseMessage: Blob) => {
        const file = new Blob([responseMessage], { type: 'application/pdf' });
 
        if (file.size === 0) {
          console.warn('Empty PDF file received');
          this.form16_mob = null;
          return;
        }
  ...
  availableYears: { year: string, label: string, isCurrent?: boolean }[] = [];
  ...
  showForm16Modal() {
    ...
    this.form16_mob = null;
 this.isLoading=true;
    this.authService.form16List(this.userData.user.empID).subscribe((res: any) => {
   // this.authService.form16List('13584').subscribe((res: any) => {
      const item = res.Form16list[0];
      this.availableYears = [];
 
      if (item.firstYear !== 'NA') {
        this.availableYears.push({ 
          year: item.firstYear, 
          label: item.displayNameFirstYear, 
          isCurrent: true
        });
      }
      
      if (item.secondYear !== 'NA') {
        this.availableYears.push({ year: item.secondYear, label: item.displayNameSecondYear });
      }
      if (item.thirdYear !== 'NA') {
        this.availableYears.push({ year: item.thirdYear, label: item.displayNameThirdYear });
      }
      this.isLoading=false;
...
async handleNoticeAndRefresh() {
  ...
  ngOnInit(): void {  
   ...
   // this.loggedUser = atob(localStorage.getItem('userData'));
    ...
    //console.log("worksheesublocation" ,   worksheesublocation)
    //console.log("worksheetoffice" ,   worksheetoffice)

    //this.worksheet=this.userData.user.buid
   // alert(this.userData.user.SUBLOCATION);
    ...
    //console.log(this.worksheet+"---");
     ...
     //console.log("BU ID>>>>>>>>>>>>" ,   this.userData.user.buid)
   ...
   // this.myDate = atob(localStorage.getItem('currentDate')); 
    ...
    if (this.userData.user.empID) {
      this.authService.getEmployeeInsurance(this.userData.user.empID).subscribe({
        next: (res) => {
          console.log('Insurance API response:', res);
   
          const hasSelf = res?.fileContent?.trim() !== '' && res?.fileName !== 'NA';
          const hasFamily = res?.familyFileContent?.trim() !== '' && res?.familyFileName !== 'NA';
         
          this.hasInsuranceFiles = hasSelf || hasFamily;
         
          this.hasInsuranceFiles = hasSelf || hasFamily;
          this.insuranceData = res;
   
          // Save to localStorage so InsuranceDetailsComponent can read
          localStorage.setItem('insuranceData', JSON.stringify(res));
        },
        error: (err) => {
          console.error('Insurance API error:', err);
          this.hasInsuranceFiles = false;
        }
      });
    }
...
//************login time substraction in seconds*******
  ...
  goToInsurance(): void {
    if (this.hasInsuranceFiles) {
      this.router.navigate(['/insurance-details']);
    } else {
      alert('Insurance files not available.');
    }
  }
