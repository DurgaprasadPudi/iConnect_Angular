...
export class AuthService {    
  ...
  //  imgbase:any = "http://192.168.30.223:8000/"; 
   ...
   // Local path   
      //baseUrl: any = "http://192.168.212.138:8094/";  
     // imgbase:any = "http://192.168.212.138:8094/";
  
     //  baseUrl: any = "http://192.168.212.75:8094/"; 
     // imgbase:any = "http://192.168.212.75:8094/";
...
getDepartmentDisplay(reportingId: any):Observable<any[]>{
  //console.log('Calling API with reportingId:', reportingId);
  return this.http.get<any[]>(`${this.baseUrl}worksheet/dept/${reportingId}`);
}
...
getNotifications(formData: FormData): Observable<any> {
  return this.http.post(`${this.baseUrl}/HRMS/notifications`, formData);
}
//// password reset
...
getemployeeBusinessUnit(formdata:FormData): Observable<any> {
 ...
 getWorksheetDownload(formData: FormData): Observable<Blob> {
  return this.http.post(this.baseUrl + '/worksheetsheetreport/download', formData, { responseType: 'blob' });
}
getSelfDownload(formData: FormData): Observable<Blob> {
  return this.http.post(this.baseUrl + '/worksheetsheetreport/self', formData, { responseType: 'blob' });
}
getWorksheetEmployees(formData:FormData):Observable<any>{
  return this.http.post(this.baseUrl+'/worksheetsheetreport/getEmployees',formData);
}
getWorksheetDataByEmployeeId(formData:FormData):Observable<any>{
  return this.http.post(this.baseUrl+'/worksheetsheetreport/getEmployeesbyid',formData);
} 
getTeamsByEmployeeid(formData:FormData):Observable<any>{
  return this.http.post(this.baseUrl+'/worksheetsheetreport/getTeams',formData);
}

//// Requisition from

getRequisitionEmpData(employeeId: any): Observable<any> {
  return this.http.get<any>(`${this.baseUrl}/form/empdetails/${employeeId}`);
}
getRequisitionBu(formdata:FormData):Observable<any>{
  return this.http.post<any>(`${this.baseUrl}/form/bu`,formdata);
}
getRequistionDepartment():Observable<any>{
  return this.http.get<any>(`${this.baseUrl}/form/department`);
}
 
getRequistionDesignation():Observable<any>{
  return this.http.get<any>(`${this.baseUrl}/form/designation`);
}
submitRequistion(data:any):Observable<any>{
  return this.http.post<any>(`${this.baseUrl}/form/insertmpform`,data)
}
getUnitNames(formdata:FormData):Observable<any>{
  return this.http.post<any>(`${this.baseUrl}/form/name`,formdata);
}
 submitVacantPositions(data: any) {
    return this.http.post(`${this.baseUrl}/form/positions`, data);
  }

  //// ID Card

  saveIdcardHistory(data: any): Observable<any> {
    return this.http.post(`${this.baseUrl}/user/uploaddata`, data);
  }
 
  updateIdcardHistory(data: any): Observable<any> {
    return this.http.post(`${this.baseUrl}/user/updateiddetails`, data);
  }
 
  getIdcardHistory(status: any): Observable<any[]> {
    return this.http.get<any[]>(`${this.baseUrl}/user/fetchdetails/${status}`);
  }
  viewIssueDetails(trackingId: any): Observable<any> {
    return this.http.post(`${this.baseUrl}/user/getreissueddetails/${trackingId}`, {});
  }
//form16 
public form16List(empId: string) {
  return this.http.get(`${this.baseUrl}/employeeflexi/Form16list/${empId}`);
}
public getForm16Pdf(year: string, pan: string) {
  return this.http.get(`${this.baseUrl}/employeeflexi/FY/form16/${year}/${pan}`, { responseType: 'blob' });
}
/////

saveInsuranceData(formData:any): Observable<any[]> {
  return this.http.post<any>(`${this.baseUrl}/insurance/filesupload`, formData)
}

getInsuranceFiles(type:number): Observable<any[]> {
  return this.http.post<any[]>(`${this.baseUrl}/insurance/files/${type}`,{});
}

//employee insurance details
getEmployeeInsurance(empid: any): Observable<any> {
  return this.http.post<any>(`${this.baseUrl}/insurance/emp/${empid}`, {});
}
getInsuranceInfo(): Observable<{ [filename: string]: string }> {
  return this.http.get<{ [filename: string]: string }>(`${this.baseUrl}/insurance/usermanual`);
}


getStatusCodes(): Observable<any[]> {
  return this.http.get<any[]>(`${this.baseUrl}/report/statuscode`);
}
 
getMasterDetails(payload: any): Observable<any[]> {
  const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
  return this.http.post<any[]>(`${this.baseUrl}/report/masterdetails`,payload,{ headers });
}
